...
const shouldUseNativeCodeLoading = boolish(
  "EXPO_WEBPACK_USE_NATIVE_CODE_LOADING",
  false
);
...
export default async function (
  ...
  const templateIndex = parse(
    readFileSync(locations.template.indexHtml, { encoding: "utf8" })
  );
  ...
  const [manifestLink] = links.filter(
    (link) =>
      typeof link.rel === "string" && link.rel.split(" ").includes("manifest")
  );
  ...
  let webpackConfig: DevConfiguration = {
      ...
      !isNative &&
        new ManifestPlugin({
          ...
          generate: (seed: Record<string, any>, files, entrypoints) => {
            const manifestFiles = files.reduce<Record<string, string>>(
              (manifest, file) => {
                if (file.name) {
    ...
    performance: isCI
      ? false
      : { maxAssetSize: 600000, maxEntrypointSize: 600000 },
  };
  ...
  if (!isNative) {
    webpackConfig = withNodeMocks(
      withAlias(webpackConfig, getAliases(env.projectRoot))
    );
  }
